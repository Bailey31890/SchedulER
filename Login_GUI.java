	...
	public Login_GUI(SchedulERlauncher go, FileIO inOut) {
		...
		submitButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					String[][] userArray = inOut.getUserInfo();
					
					
					for(int i=0; i<userArray.length; i++) {
						//loop through user array						
						if((userField.getText().equals(userArray[i][1])) && (userArray[i][2].equals(passwordField.getText()))){
							//username and password are in user info in same row - user is verified
							
							int userID = Integer.parseInt(userArray[i][0]);
					...
					System.out.println("FileIO cannot find proper user information file.");
					e.printStackTrace();
				}
				
				//user and pass are not matched in userArray
				if(!userFound) {
				showAlert(Alert.AlertType.ERROR, loginGridpane.getScene().getWindow(), "Input Error", "Incorrect Username or Password. Please Retry.");
				return;
				}
